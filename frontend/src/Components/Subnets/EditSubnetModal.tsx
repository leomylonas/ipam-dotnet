...
export function EditSubnetModal({ subnet, onClose, tenancyId }: EditSubnetModalProps) {
	const { useUpdatePrivate } = useSubnets();
	const mutation = useUpdatePrivate(tenancyId);
	const {
		register,
		handleSubmit,
		reset,
	...
	const onSubmit = handleSubmit(async (data) => {
		...
		await mutation.mutateAsync({ subnetId: subnet.id, request: data });
		handleClose();
	});
